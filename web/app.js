import {createDraftStore,captureDraft,draftChanges} from './draft-store.js';
...
let historyId=null,savedSnapshot=null,lockDiagnosis=null,excerptOrder=[];
...
async function api(action,data={},signal){const body=JSON.stringify({action,data});if(new TextEncoder().encode(body).byteLength>24*1024*1024)throw Object.assign(new Error('本次操作超过 24 MiB；请减少单次引用内容，或使用 CLI open 文件入口。当前草稿保留。'),{code:'INPUT_TOO_LARGE'});const response=await fetch('/api',{method:'POST',headers:{'Content-Type':'application/json','x-relay-token':token},body,signal});const out=await response.json();if(!response.ok)throw Object.assign(new Error(out.error?.message||'请求失败'),{code:out.error?.code||'HTTP_ERROR'});return out;}
async function run(work){try{await work();}catch(e){notice(`${e.code||'操作失败'} · ${e.message}。当前草稿仍保留。`);}}
const drafts=createDraftStore({copy:snapshot=>snapshot,save:async snapshot=>{
  const changes=draftRevision?draftChanges(savedSnapshot,snapshot):null;
  if(changes&&Object.keys(changes).length===0)return;
  const result=await api(changes?'draft-patch':'draft-save',{...(changes?{changes}:snapshot),draftId,expectedRevision:draftRevision});
...
function persist(){if(draftReady)void drafts.update(captureDraft({historyId,pack,selectionCounter,excerptOrder:displayExcerpts().map(x=>x.id),question:$('question').value}));}
...
function clearLockDiagnosis(){lockDiagnosis=null;$('lock-recover').disabled=true;$('lock-diagnose').disabled=!receipt;$('lock-status').textContent='';}
function mutate(){preview=null;receipt=null;clearLockDiagnosis();$('send').disabled=true;$('reconcile').disabled=true;persist();}
...
async function commitSelection(selectedHistory,m,start,end){
  if(selectedHistory!==history)throw new Error('历史已变化，请在当前原文重新选择');
  ...
  const result=await api('pack',{historyId,selections:[{localId:m.localId,start,end}],question:$('question').value});
  if(selectedHistory!==history)throw new Error('历史已变化，旧选段未写入当前稿');
...
function displayExcerpts(){if(!pack)return [];const byId=new Map(pack.excerpts.map(x=>[x.id,x]));return [...excerptOrder.filter(id=>byId.has(id)).map(id=>byId.get(id)),...pack.excerpts.filter(x=>!excerptOrder.includes(x.id))];}
function moveExcerpt(id,offset){const list=displayExcerpts().map(x=>x.id);const from=list.indexOf(id),to=from+offset;if(to<0||to>=list.length)return;[list[from],list[to]]=[list[to],list[from]];excerptOrder=list;renderExcerpts();mutate();notice('已调整引用篮展示顺序；引用编号和发送正文仍按原引用顺序。');}
...
function renderExcerpts(){const host=$('excerpts');host.replaceChildren();$('count').textContent=`${pack?.excerpts.length||0} 条引用`;
  ...
  for(const [index,x] of displayExcerpts().entries()){const row=node('article',undefined,'excerpt');const h=node('div',undefined,'excerpt-head');const a=node('a',`[${x.label}] · ${roles[x.role]}`);a.href='#';a.onclick=e=>{e.preventDefault();showSource(x)};h.append(a);for(const [offset,label]of [[-1,'上移'],[1,'下移']]){const b=node('button',label,'plain small');b.title='仅调整展示顺序，发送时保持引用编号';b.setAttribute('aria-label',`${label}${x.label}`);b.disabled=index+offset<0||index+offset>=pack.excerpts.length;b.onclick=()=>moveExcerpt(x.id,offset);h.append(b);}const del=node('button','移除','plain small');del.setAttribute('aria-label',`移除${x.label}`);del.onclick=()=>removeExcerpt(x.id);h.append(del);row.append(h,node('blockquote',x.exactText),node('div',`${x.sourceItemId||'消息 ID 未知'} · ${x.range.start}–${x.range.end} · ${x.sourceUri||'来源未知'}`,'small'));host.append(row);}
}
...
async function applyImport(result){if(result.draft){({history,pack,selectionCounter}=result.draft);excerptOrder=result.draft.excerptOrder||[];historyId=result.draft.historyId||null;$('question').value=result.draft.question;}else if(result.pack){pack=result.pack;excerptOrder=[];history=null;historyId=null;selectionCounter=Math.max(...pack.excerpts.map(x=>x.selectionOrder));$('question').value=pack.question;}else {history=result.history;historyId=result.historyId||null;}if(history&&!historyId)({historyId}=await api('history-store',{history}));await hashHistory();render();mutate();notice(result.fixture?'当前是离线演示 fixture，不是宿主对话，也没有真实模型输出。':(result.history?.warnings||[]).join(' '));}
$('file').onchange=()=>run(async()=>{const f=$('file').files[0];if(!f)return;try{const text=await f.text();let parsed;try{parsed=JSON.parse(text.replace(/^\uFEFF/,''));}catch{}let result;if(parsed?.draftFormat==='context-relay-draft/v1'&&parsed.history){
  // A complete backup contains both source and selected text. Validate the
  // compact half first, then store history separately without a 2x HTTP body.
  result=await api('draft-validate',{...parsed,history:null});
  const stored=await api('history-store',{history:parsed.history});result.draft.history=parsed.history;result.draft.historyId=stored.historyId;
}else result=await api('history-import',{text,sourceUri:`user-selected-file:${f.name}`});await selectionQueue.catch(()=>{});if((result.pack||result.draft)&&(pack?.excerpts.length||$('question').value)&&!confirm('载入此文件会替换当前引用、背景和问题。继续？'))return;await applyImport(result);}finally{$('file').value='';}});
...
$('verify').onclick=()=>run(async()=>{requirePack();if(!history)throw new Error('请先导入相应原历史；引用快照仍保留');const r=await api('sources',{pack,historyId});notice(r.sources.map(x=>`${x.label}：${{unchanged:'原文一致',changed:'原文已变化，固定快照保留',missing:'无法定位，固定快照保留'}[x.status]}`).join('；'));});
...
$('read-thread').onclick=()=>run(async()=>{const threadId=$('source-thread').value.trim();if(!threadId)throw new Error('请选择已有对话或填写明确的已有对话 ID');let result;try{result=await api('history-thread-read',{threadId});}catch(e){showConnection({mode:'export-only',reason:e.message,readVerified:false});throw e;}await selectionQueue.catch(()=>{});await applyImport(result);if(result.connection)showConnection(result.connection);});
function showReceipt(r){receipt=r;clearLockDiagnosis();if(r.preview)$('prompt').textContent=r.preview;$('receipt').textContent=`${stateLabels[r.status]||r.status}\n目标：${r.targetThreadId}\n回合：${r.turnId||'尚无回合 ID'}\n${r.note||''}${r.error?'\n'+r.error.code+' · '+r.error.message:''}\n回执：${r.id}`;$('send').disabled=!(r.status==='prepared'||r.status==='failed'&&!r.deliveryAttempted);$('reconcile').disabled=!['submitted','unknown'].includes(r.status);}
...
$('target').oninput=()=>{receipt=null;clearLockDiagnosis();$('send').disabled=true;$('reconcile').disabled=true;$('receipt').textContent='目标已变化，请重新准备转发。';};
...
$('lock-diagnose').onclick=()=>run(async()=>{const id=receipt.id;const result=await api('receipt-lock-diagnose',{receiptId:id});if(receipt?.id!==id)return;lockDiagnosis=result;$('lock-status').textContent=`${result.code} · ${result.reason}\n${result.recoverable?'可明确解除失效的发送前锁；此操作不会发送正文。':'当前不能解除；不会重发。'}`;$('lock-recover').disabled=!result.recoverable;});
$('lock-recover').onclick=()=>run(async()=>{if(!lockDiagnosis?.recoverable||lockDiagnosis.receiptId!==receipt?.id)throw new Error('请先诊断当前回执');const result=await api('receipt-lock-recover',{receiptId:receipt.id,expectedLockToken:lockDiagnosis.lockToken,confirm:true});clearLockDiagnosis();$('lock-status').textContent=result.note;$('send').disabled=receipt.deliveryAttempted||!['prepared','failed'].includes(receipt.status);});
...
await run(async()=>{try{({token}=await (await fetch('/bootstrap')).json());const draft=await api('draft-load',{draftId});draftRevision=draft.revision;history=draft.history;historyId=draft.historyId||null;if(history&&!historyId)({historyId}=await api('history-store',{history}));pack=draft.pack;excerptOrder=draft.excerptOrder||[];$('question').value=draft.question??pack?.question??'';selectionCounter=Math.max(draft.selectionCounter||0,...(pack?.excerpts.map(x=>x.selectionOrder)||[0]));await hashHistory();render();savedSnapshot=captureDraft({historyId,pack,selectionCounter,excerptOrder:displayExcerpts().map(x=>x.id),question:$('question').value});draftReady=true;document.querySelector('main').inert=false;$('draft-location').textContent=draftId==='main'?'默认草稿':'独立草稿 · 此地址可恢复';if(pack||draft.question)$('draft-status').textContent='已恢复草稿';}catch(error){$('boot-error').hidden=false;$('boot-message').textContent=error.message;throw error;}});
